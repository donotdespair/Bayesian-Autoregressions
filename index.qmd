...
This integration produces a density function which is proportional to the distribution for a Student-t distribution with $nu$ degrees of freedom. Hence, $x\sim t_\nu$.
...
As before, we can derive the full conditional posteriors of the parameters by multiplying the likelihood and the priors. The natural conjugacy of $\alpha$ and $\sigma^2$ are preserved under this specification and therefore the joint posterior is given by
...
\overline s_\lambda &= \frac{1}{\sigma^2}(\mathbf{y - X\alpha})'(\mathbf{y - X\alpha}) + \underline s_\lambda \\
