...
Alternatively, the scalar scale $\kappa_\alpha$ that premultiplies the covariance matrix of the normal prior for vector $\alpha$ can be assumed to have a hierarchical prior with a gamma distribution:
$$
\kappa_\alpha|\underline s_\kappa, \underline a_\kappa \sim \mathcal{G}(\underline s_\kappa, \underline a_\kappa)
$$
The following code creates a list `kappa.priors` to store priors on $\kappa_\alpha$.
...
Given the likelihood and priors above, we can obtain the **full conditional posterior** of $\kappa_\alpha$ as
$$
\begin{align*}
...
$$
\begin{align*}
    & \lambda = \underline a_{\boldsymbol\kappa} - \frac{K}{2}\\
    & \chi = (\boldsymbol\alpha-\underline{\boldsymbol\alpha})^{'} (\sigma^2 \underline{\mathbf{V}}_{\boldsymbol\alpha})^{-1} (\boldsymbol\alpha-\underline{\boldsymbol\alpha}) \\
    & \Psi = \frac{2}{\underline s_{\boldsymbol\kappa}}
\end{align*}
$$
Hence, the **full-conditional posterior** distribution of $\kappa_\alpha$ follows a  Generalised Inverse Gaussian distribution.
$$
\kappa_{\boldsymbol\alpha} | \bf y, \bf X, {\boldsymbol\alpha}, \sigma^2 \sim \mathcal{GIG}(\lambda, \chi, \Psi)
$$
