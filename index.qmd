...
$$\bar{y}_t = \bar{\mathbf{x}}_t'\boldsymbol\alpha + u_t$$ where
$\bar{y}_t = y_t/c_t$ and $\bar{\mathbf{x}}_t = \mathbf{x}_t/c_t$. These
...
Then, after substituting in data for y and X, the likelihood function
for estimation of the parameters is given by:
...
where $\tilde{\mathbf{x}}_t=\frac{(1,\mathbf{y}_t,...,\mathbf{y}_{t-p})'}{\mathbf{c}_t}$,
$\tilde{\hat{A}}=(\tilde{\mathbf{X}}'\tilde{\mathbf{X}}-\underline{V}^{-1})^{-1}$, and
$\underline{\gamma}=(\underline{A},\underline{S},\underline{V},\underline{\nu})$

and the priors are assumed to be: $\mathbf{c}_0,\mathbf{c}_1,\mathbf{c}_2\sim Pareto(1,1)$ and
$\rho\sim Beta(3,1.5)$
