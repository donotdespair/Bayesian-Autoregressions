...
\begin{align}
p(y_{t+1} |x_{t+1},\bf y, \bf X) &= \int\int p(y_{t+1} |x_{t+1}, Y, X, \boldsymbol\alpha, \sigma^{2})p({\boldsymbol\alpha, \sigma^{2}bf y,\bf X)d{\boldsymbol\alpha d\sigma^{2}\\
p(y_{t+1} |x_{t+1}, \bf y,\bf X, \boldsymbol\alpha, \sigma^{2}) &= \mathcal{N}( \alpha'_{d}d_{t+1|t} + \alpha_{1}y_{t}+\dots+\alpha_{p}y_{t-p+1}, \sigma^{2})\\
p({\boldsymbol\alpha, \sigma^{2}|\bf y,\bf X) &= \mathcal{NIG}2(\overline{\boldsymbol\alpha}, \overline{\sigma^{2}}, \overline{S}, \overline{v}) \\
\end{align}
...
Sample draws from $p(y_{t+1} |x_{t+1})$ that is specified as $y_{t+1} \sim \mathcal{N} (y_{t+1|t}(\boldsymbol\alpha^{(s)}), Var[y_{t+1|t}|\boldsymbol\alpha^{(s)},  \sigma^{2(s)}])$
