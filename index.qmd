...
## Comparison of Frequentist and Bayesian forecasting

In the frequentist approach, we condition the predictive density on $\theta$, the parameters. The latters are treated as non-random.  Therefore, it can be stated that this approach ignores estimation uncertainty. 

To solve this issue, the Bayesian approach treats the parameters $A$ and $\Sigma$ as random variables and thus, it incorporates estimation uncertainty into the predictive density. In fact, if we impose appropriate priors, Bayesian forecasting often leads to more precise estimations as it narrows the confidence intervals by using priors. 

Moreover, Bayesian forecasting uses the frequentist approach as a starting point. In the following formula for the conditional one-period ahead forecast, the first expression under the integral signs is the conditional predictive density of the frequentist approach and the second expression is the joint posterior distribution that we are now used to derive. 

$$
\begin{aligned}
p\left(y_{t+1} \mid x_{t+1}, Y, X\right) & =\iint p\left(y_{t+1} \mid x_{t+1}, Y, X, A, \Sigma\right) p(A, \Sigma \mid Y, X) d A d \Sigma \\
p\left(y_{t+1} \mid x_{t+1} Y, X, A, \Sigma\right) & =\mathcal{M} \mathcal{N}_{1 \times N}\left(x_{t+1}^{\prime} A, \Sigma, 1\right) \\
p(A, \Sigma \mid Y, X) & =p(A \mid Y, X, \Sigma) p(\Sigma \mid Y, X) \\
p(A \mid Y, X, \Sigma) & =\mathcal{M} \mathcal{N}_{K \times N}(\bar{A}, \Sigma, \bar{V}) \\
p(\Sigma \mid Y, X) & =\mathcal{I W}_N(\bar{S}, \bar{\nu})
\end{aligned}
$$
To derive the solution of this expression analytically, we integrate the parameters out in this order: 

1. We integrate out $A$.

By integrating out $A$, we obtain:

$$
p\left(y_{t+1} \mid x_{t+1}, Y, X, \Sigma\right)=\mathcal{M N}_{1 \times N}\left(x_{t+1}^{\prime} \bar{A}, \Sigma, 1+x_{t+1}^{\prime} \bar{V} x_{t+1}\right)
$$

2. We integrate out $\Sigma$

By integrating out $\Sigma$, we obtain:
$$
p\left(y_{t+1} \mid x_{t+1}, Y, X\right)=\mathcal{M} t_{1 \times N}\left(x_{t+1}^{\prime} \bar{A}, 1+x_{t+1}^{\prime} \bar{V} x_{t+1}, \bar{S}, \bar{\nu}\right)
$$
