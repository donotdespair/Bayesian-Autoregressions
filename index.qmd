...
$$\textbf{u}|\textbf{X}\sim \mathcal{N}_T(0,\sigma^2\text{diag}(C))$$

This first step is to sample draws of the vector of COVID volatility
variables $C=[1\quad...\quad1 \quad \bar{c}_0\quad \bar{c}_1\quad \bar{c}_2\quad 1+(\bar{c}_2-1)\rho\quad 1+(\bar{c}_2-1)\rho^2\quad...]'$.

This is done by estimating parameters in $\theta=(\bar{c}_0,\bar{c}_1,\bar{c}_2,\rho)$ by
sampling from their own marginal posterior via a Metropolis MCMC
algorithm:

1.  Initialize $\theta$ at the posterior mode which is can be located
    ...
    representing matrix $\textbf{y}$ and returns values for $\theta$
...
2.  Draw candidate $\theta^{*}$ from $N_4(\theta^{(s-1)},cW)$, where $W$
    is the inverse Hessian of the negative log posterior of $\theta$ at
...
4.  Define $C^{(s)}$ matrix using $\theta^{(s)}$:

$$C^{(s)}=[1\quad...\quad1 \quad \bar{c}_0^{(s)}\quad \bar{c}_1^{(s)}\quad \bar{c}_2^{(s)}\quad 1+(\bar{c}_2^{(s)}-1)\rho^{(s)}\quad 1+(\bar{c}_2^{(s)}-1)\rho^{(s)2}\quad...]'$$

Steps 2 to 4 are implemented via the function, `mh.mcmc` which takes in
data, the posterior mode of $\theta$, and the inverse Hessian from
`c.posterior.mode` and returns the draws of $\theta$.
...
5.  These $\theta$ draws, are then used to draw $\boldsymbol{\alpha}$
    and $\Sigma$ from the following posterior distribution:

$$P(\boldsymbol{\alpha}|Y,X,\Sigma,C)=MN_{K\times N}(\bar{\boldsymbol{\alpha}},\Sigma,\bar{V})$$
$$P(\Sigma|Y,X,C)=IW_N(\bar{S},\bar{\nu})\\$$

$$\bar{V}=(X'\text{diag}(C^2)X+\underline{V}^{-1})^{-1}$$
$$\bar{\boldsymbol{\alpha}}=\bar{V}(X'\text{diag}(C^2)Y+\underline{V}^{-1}\underline{\boldsymbol{\alpha}})$$
$$\bar{\nu}=T+\underline{\nu}$$
$$\bar{S}=\underline{S}+Y'\text{diag}(C^2)Y+\underline{\boldsymbol{\alpha}}'\underline{V}^{-1}\underline{\boldsymbol{\alpha}}-\bar{\boldsymbol{\alpha}}'\bar{V}^{-1}\bar{\boldsymbol{\alpha}}$$
...
The implementation of the above estimation procedure is demonstrated
below, on an independent bi-variate Gaussian random walk with drift
series. Both series have 128 observations, exhibit an increasing
stochastic trend due to a drift term of $\mu=1$, and were generated to
have increased volatility (i.e., three times the standard deviation) on
periods $t\in[129,136]$ before returning to a standard deviation of 1 in
the last 4 periods. This artificially generated series aims to mimic unit-root non-stationary macroeconomic variables that experiences heightened volatility during COVID-19.
...
# Generate RW series with drift and higher variance at covid periods.
RW1 <- ts(cumsum(c(1+rnorm(116, mean = 0, sd = 1),1+rnorm(8, mean = 0, sd = 3),1+rnorm(4, mean = 0, sd = 1))))
RW2 <- ts(cumsum(c(1+rnorm(116, mean = 0, sd = 1),1+rnorm(8, mean = 0, sd = 3),1+rnorm(4, mean = 0, sd = 1))))
Y <- cbind(RW1, RW2)

# Plot the series
par(mfrow=c(1,2), mar=c(2,2,2,2))
plot(y=RW1, x = 1:128, type = "l", col = "turquoise4", 
     lwd = 1.5, ylab = "", xlab = "", main = "RW with drift 1")
plot(y=RW2, x = 1:128, type = "l", col = "plum4", 
     lwd = 1.5, ylab = "",xlab = "", main = "RW with drift 2")
```

First, `posterior.mode` and `mh.mcmc` are used to draw COVID volatility vectors $C^{(s)}$. As seen in the plots below, MCMC draws of $\theta$ converges to a somewhat stationary series after 22,000 draws.

```{r RW Metropolis MCMC, echo = T}
#| code-fold: true
#| code-summary: "show code"

# Locate posterior mode of COVID volatility  
RW.post.mode <- c.posterior.mode(Y, p = 1)

# Run Metropolis MCMC
set.seed(1291042)
RW.theta <- mh.mcmc(Y, p = 1, S.mh = 22000, c = 0.1, 
        W = solve(RW.post.mode$hessian), theta.init = RW.post.mode$maximizer)

# Plot theta draws
plot.ts(RW.theta$Theta, main = "Metropolis MCMC draws")
```

Using the last 10,000 MCMC draws, `covid.est` is ran to draw $\boldsymbol{\alpha}$ and $\Sigma$. As seen in the results table below, the estimated posterior means of $\boldsymbol{\alpha}$ and $\Sigma$ are relatively close to the true values, except for $\alpha_{11}$ which is quite far from the true value of 1.

```{r RW Estimation, echo = T}
#| code-fold: true
#| code-summary: "show code"
RW.est <- covid.est(Y, p = 1, S = 10000, Theta.mh = RW.theta$Theta[12001:22000,]) 

A.mean_tab <- as.data.frame(round(apply(RW.est$A.posterior, 1:2, mean),2))
colnames(A.mean_tab) <- c("RW1", "RW2")

Sigma.mean_tab <- as.data.frame(round(apply(RW.est$Sigma.posterior, 1:2, mean),2))
