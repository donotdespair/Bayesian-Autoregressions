...
$$
L(A,\Sigma|Y,X,h) = det(diag(h)^{-N/2}det(\Sigma)^{-T/2}\times exp\{  -1/2 [\Sigma ^ {-1} (Y-\alpha X)' diag(h)^{-1}(Y-\alpha X)] \}
$$
The full conditional posterior distribution for $\alpha,\Sigma$ is given by
$$
P(A,\Sigma|Y,X,h) = MN (\bar{A},\bar{V},\bar{S},\bar{\nu})
$$
where
$$
...
$$
$$
\bar{A}=\bar{V}(X'diag(h)^{-1}Y+\underline{V}^{-1}\underline{A})
$$
$$
\bar{S}=\underline{V}+Y'diag(h)^{-1}Y+\underline{A'}\underline{V}^{-1}\underline{A}-\bar{A'}\bar{V}^{-1}\bar{A}
$$
$$
\bar{\nu}= T + \underline{\nu}
$$
...
1. Auxiliary mixture @omori2007stochastic

2. Precision sampler @chan2009efficient

1. Initialize the algorithm by proposing starting values for $h^{(0)},s^{(0)},\mu_0^{(0)},\alpha^{(0)},\sigma_v^{2(0)}$
2. For each iteration i of the Gibbs sampling algorithm, draw
$$
h_{0}^{(i)} \sim N(\bar{h_0},\bar{\sigma}^2_h) 
$$ 
$$
\sigma_v^{2(i)} \sim IG2(\bar{s},\bar{v})
$$
$$
\mu_{0}^{(i)} \sim N(\bar{\mu_0},\bar{\sigma}^2_\mu) 
$$
$$
\alpha^{(i)} \sim N(\bar{\alpha},\bar{\sigma}^2_\alpha) 
$$
$$
s_t^{(i)}\sim Multinomial (\{m\}_{m=1}^{10}, \{Pr[s_t = m|\tilde{y}_t,h_t^{(i)}\}_{m=1}^{10})
$$ 
using inverse transform method 

$$
h^(i) \sim N_t(\bar{h},\bar{V}_h)
$$
using precision sampler
3. Return a sample drawn from the posterior distribution:
$$
\{h^{(i)},s^{(i)},\mu_{0}^{(i)},\alpha^{(i)},h_0^{(i)}, \sigma_v^{2(i)}\}
$$

### Gibbs sampler

At each iteration s:
   Sample $h^{S}$ from the distribution,
   then Sample $A^{S},\Sigma^{s}$ from the $P(A,\Sigma|Y,X,h) = MN (\bar{A},\bar{V},\bar{S},\bar{\nu})$
Return $\{A^{S},\Sigma^{S},h^{S}]\}_{s=1}^S$

Technices for sampling h is listed in the previous section.
