...
In this extended model I augment the system using the 'sum of coefficients' and 'dummy initial observation' prior. The sum of coefficients prior takes the average of the lagged values and adds them to the basic equation. This is because these values are assumed to be a good forecast of future observations.
...
Firstly the average of lagged values needs to be calculated, $y.bar$. This is done by finding the mean of the first $p$ values in the data. Next, values of of the scaling factors need to be selected, typically values equal to 1 are chosen.

Once this has been done, the rows can be created. In the univariate case this is simple, two rows are added to Y, the first equal to $y.bar$ divided by the first scaling factor, the second equal to $y.bar$ divided by the second scaling factor.

Adding rows to X is slightly more complicated. The additional rows are as follows. The values in the first row, corresponding to the sum of coefficients, are all equal to $y.bar$ divided by the first scaling factor, except the last column, which is equal to zero. The values in the second row, corresponding to the dummy initial observation, are all equal to $y.bar$ divided by the second scaling factor, except the last column, which is equal to one.
...
ext.data <- prior.ex(y, p = 5, lambda_3 = 1, lambda_4 = 1)
