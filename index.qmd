...
able to be easily compared, or that the most severe changes for COVID
would best be discounted due to their warping effect on the overall
data. However, Michele Lenza and Giorgio E. Primiceri disagree. In their paper *How to estimate a vector autoregression after March 2020*, rather
...
used. For higher frequency data the exact effect is unknown though should cover at least the first 3 months of the pandemic. Thus, they propose the following change to the standard formula for calculating structural variable auto regressions.

$$y_t = X_t\alpha + c_tu_t$$

Where y~t~ is the vector of parameters of interest.
$$X_t = I_t \otimes x'_t~$$ 
$$x_t = [1\quad y'_{t-1}\quad ... \quad y'_{t-p}]$$
$$\alpha = [\alpha'_d\quad \alpha_1\quad...\quad\alpha_p]'$$
u~t~ is the error term.
c~t~ is a variance multiplier. That is, for every
period before COVID it takes a value of 1 and for the first 3 periods of
COVID it takes values \bar{c}~0~, \bar{c}~1~, \bar{c}~2~, and then in all future periods a value of 
$$c_{t*+j} = 1 + (\bar{c}_2 - 1)\rho^{j-2} $$
This creates a vector that leaves the error term unchanged before COVID, has a surge in volatility during COVID, and then decays geometrically after COVID. This approximates the observed shocks to volatility.

By dividing both sides by c~t~ this can be rewritten as 
$$\bar{y}_t = \bar{X}_tα + u_t$$
where $\bar{y}_t = y_t/c_t$ and $\bar{X}_t = X_t/c_t$. These simple transformations then lend themselves to analysis using whatever method is prefered.

The main difficulty arises in the estimation of c~t~ as it is an unknown parameter in many cases.

### Methodology

To estimate the values for c~0~, c~1~, and c~2~. $C$ is a vector of COVID volatility variables:

$$C=[1\quad...\quad1 \quad c_0\quad c_1\quad c_2\quad 1+(c_2-1)\rho\quad 1+(c_2-1)\rho^2\quad...]'$$

Intuitively, volatility variables for periods before COVID are set to unity. Heightened volatility during the first three quarters of COVID are parameterized, then they are assumed to decay at a geometric rate beginning at the fourth quarter since the pandemic's onset.

These COVID volatility parameters $\theta=(c_0\quad c_1\quad c_2\quad\rho)$ are estimated from their own marginal posterior as proposed by Lenza & Primiceri (2020):
